...
import { SlashCommandBuilder, TextInputStyle, ModalBuilder, EmbedBuilder, TextInputBuilder, ActionRowBuilder, AttachmentBuilder, PermissionFlagsBits } from 'discord.js';
...
export default class extends SlashCommands {
  ...
  run = async (interaction) => {
    ...
    const info = cache.find(item => item.name.includes(test));

    if (!info) {
        interaction.reply({ content: `${this.emoji.rs} **|** Cuidado, herói! Parece que você escolheu uma opção fora da Batcaverna: \`${test}\`. Tente novamente com uma escolha válida para proteger Gotham City! 🦇💥`, ephemeral: true });
        return;
    }
